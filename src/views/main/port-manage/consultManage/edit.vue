...
<script>
import { consultRegisterUpdate } from '../../../../service/main/main'
...
export default {
  ...
  methods: {
    ...
    dataInit() {
      ...
      const { description, content, company } = this.detail
      this.registerForm = Object.assign(
        {},
        { description, content, company, registrationId: this.detail.id, applicants: [] }
      )
    ...
    async save() {
      ...
      const res = await consultRegisterUpdate(this.registerForm)
      if (res.success) {
        ElMessage({
          message: '操作成功',
          type: 'success'
