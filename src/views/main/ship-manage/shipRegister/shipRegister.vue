<template>
  <div class="shipregister">
    <el-card class="box-card1" shadow="never">
      ...
      <!-- 表单 -->
      <el-form ref="registerFormRef" :model="registerForm" :rules="rules" label-width="130px">
        <el-row>
          <el-col :span="12">
            <el-form-item label="单据类型">
              <el-input disabled v-model="docketType" placeholder="请输入单据类型" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
          ...
          <el-col :span="12">
            <el-form-item label="船舶所有者" prop="shipOwner">
              <el-input
                            ...
                            <el-col :span="24">
                              <div class="read-btns">
                                <el-button size="small" type="primary" @click="readIDCard"
                                  >开始读卡</el-button
                                >
                                <!-- <el-button size="small" type="primary" @click="clearRead"
                                  >清除信息</el-button
                                > -->
                              </div>
                            </el-col>
...
<script>
import { applicantRules, rules } from './config'
// eslint-disable-next-line no-unused-vars
import { shipRegister } from '../../../../service/main/main'
import { CameraPhoto } from '../../../../components/camera-photo/index'
import { blobToBase64 } from '../../../../utils/imageTransfer'
import moment from 'moment'
import { ElMessage } from 'element-plus'
export default {
  name: 'shipRegister',
  components: { CameraPhoto },
  data() {
    return {
      registerForm: {
        applicants: [],
  ...
  methods: {
    ...
    dataInit() {
      ...
      // 单据类型
      const docket = this.$store.state.login.shipRegCategorys.find(
        (item) => item.name === this.type
      )
      this.docketType = docket.displayName
      // 证件类型
      ...
      this.registerForm.categoryId = docket.id
      this.persons = [
        {
          name: '证件人1',
          personForm: {
    ...
    async save() {
      ...
      const res = await shipRegister(this.registerForm)
      if (res.success) {
        // this.$router.push('/main/ship-manage/shipManage')
        ElMessage({
          message: '操作成功',
          type: 'success'
        })
