...
<script>
import { inspectRegisterUpdate } from '../../../../service/main/main'
...
export default {
  ...
  methods: {
    ...
    dataInit() {
      ...
      const { description, affairsName, company } = this.detail
      this.registerForm = Object.assign(
        {},
        { description, affairsName, company, registrationId: this.detail.id, applicants: [] }
      )
    ...
    async save() {
      ...
      const res = await inspectRegisterUpdate(this.registerForm)
      if (res.success) {
        ElMessage({
          message: '操作成功',
          type: 'success'
