...
import { getInspectList, deleteInspectRegister } from '../../../../service/main/main'
...
export default {
  ...
  methods: {
    ...
    detail(data) {
      ...
      this.$router.push('/main/inspect-manage/inspectManage/detail/' + data.id)
    },
    edit(data) {
      localCache.cacheSet('detail', data)
      this.$router.push('/main/inspect-manage/inspectManage/edit/' + data.id)
    },
    timeChange(event) {
    ...
    // 删除
    async remove(id) {
      const res = await deleteInspectRegister(id)
      if (res.success) {
        ElMessage({
          message: '操作成功',
          type: 'success'
