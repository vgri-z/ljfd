        ...
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="营运证编号">
              <el-input v-model="searchForm.CertNumber" placeholder="请输入营运证编号"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item>
...
import { getOperationList, deleteOperationRegister } from '../../../../service/main/main'
...
export default {
  ...
  methods: {
    ...
    async _getOperationList() {
      const obj = Object.assign({}, this.searchForm, this.searchOption)
      const res = await getOperationList(obj)
    ...
    detail(data) {
      ...
      this.$router.push('/main/operation-manage/operateManage/detail/' + data.id)
    },
    edit(data) {
      localCache.cacheSet('detail', data)
      this.$router.push('/main/operation-manage/operateManage/edit/' + data.id)
    },
    ...
    // 删除
    async remove(id) {
      const res = await deleteOperationRegister(id)
      if (res.success) {
        ElMessage({
          message: '操作成功',
          type: 'success'
